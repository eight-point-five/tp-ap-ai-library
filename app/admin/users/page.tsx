    ...
    <section className="rounded-2xl bg-white p-7">
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-light-500">
          在这里查看普通用户和管理员账号状态。已批准的新管理员同样可以被其他管理员封禁，
          但不能操作自己的账号。
        </p>
      </div>
        ...
        <table className="min-w-full text-left">
          <thead>
            <tr className="border-b border-light-400 text-sm text-light-500">
              ...
              <th className="py-3 pr-4">账号管控</th>
          ...
          <tbody>
            {allUsers.map((user) => (
              <tr key={user.id} className="border-b border-light-400/70 text-sm">
                ...
                <td className="py-4 pr-4">
                  {user.role === "USER" && user.currentLevel ? (
                    ...
                    <span className="text-light-500">管理员账号不参与风控</span>
                  )}
                </td>
                <td className="py-4 pr-4">
                  <UserStatusControls
                ...
                <td className="py-4">
                  {user.role === "USER" ? (
                    ...
                    <span className="text-light-500">管理员账号无风控详情</span>
                  )}
                </td>
              </tr>
            ))}
